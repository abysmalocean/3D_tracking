...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh  = (1.0 - np.square(next_h))
    # db
    mid = dnext_h * dtanh
    db = np.sum(mid, axis=0)
    # dx 
    dx = np.dot(mid, Wx.T)
    # dWx
    dWx = np.dot(x.T, mid)
    # dprev_h
    dprev_h = np.dot(mid, Wh.T)
    # dWh
    dWh = np.dot(prev_h.T, mid)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(T): 
        h[:,i,:], back_cache = rnn_step_forward(x[:,i,:], prev_h, Wx, Wh, b)
        prev_h = h[:, i, :]
        cache.append(back_cache)
